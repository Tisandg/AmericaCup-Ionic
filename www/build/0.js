...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_4__Match__ = __webpack_require__(284);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_5__Team__ = __webpack_require__(285);
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    ...
    MatchesPage.prototype.getMatches = function (groupId) {
        ...
        data.subscribe(function (result) {
            for (var _i = 0, _a = result.data.match; _i < _a.length; _i++) {
                ...
                var newMatch = new __WEBPACK_IMPORTED_MODULE_4__Match__["a" /* Match */](item.id, item.home_id, item.away_id, item.home_name, item.away_name, item.score, item.date, item.status, imageA, imageB);
                _this.matches.push(newMatch);
                //Agregar equipos
                if (_this.teamsId.indexOf(item.home_id) == -1) {
                    //Agregarlo
                    var newTeam = new __WEBPACK_IMPORTED_MODULE_5__Team__["a" /* Team */](item.home_id, item.home_name, imageA, groupId, 0);
                    _this.teams.push(newTeam);
                    _this.teamsId.push(item.home_id);
                }
                if (_this.teamsId.indexOf(item.away_id) == -1) {
                    //Agregarlo
                    var newTeam = new __WEBPACK_IMPORTED_MODULE_5__Team__["a" /* Team */](item.away_id, item.away_name, imageB, groupId, 0);
                    _this.teams.push(newTeam);
